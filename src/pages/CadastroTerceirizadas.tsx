...
const emptyForm: TerceirizadaForm = {
  nome: '', cnpj: '', responsavel: '', endereco: '', cidade: '', estado: 'BA', telefone: '', email: '', tipo: 'terceirizada',
};
...
const CadastroTerceirizadas = () => {
  const { toast } = useToast();
  const queryClient = useQueryClient();
  const [dialogOpen, setDialogOpen] = useState(false);
  const [editId, setEditId] = useState<string | null>(null);
  const [form, setForm] = useState<TerceirizadaForm>(emptyForm);
  const [search, setSearch] = useState('');

  const { data: terceirizadas = [], isLoading } = useQuery({
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      ...
      if (editId) {
        const { error } = await supabase.from('terceirizadas').update(payload).eq('id', editId);
        if (error) throw error;
      } else {
        const { error } = await supabase.from('terceirizadas').insert(payload);
        if (error) throw error;
      }
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['terceirizadas'] });
      toast({ title: editId ? 'Registro atualizado' : 'Registro cadastrado' });
      closeDialog();
    },
  ...
  const toggleMutation = useMutation({
    mutationFn: async ({ id, ativo }: { id: string; ativo: boolean }) => {
      const { error } = await supabase.from('terceirizadas').update({ ativo: !ativo }).eq('id', id);
      if (error) throw error;
    },
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['terceirizadas'] }),
  });
  ...
  const openEdit = (item: any) => {
    ...
    setForm({
      ...
      email: item.email || '',
      tipo: item.tipo || 'terceirizada',
    });
    setDialogOpen(true);
  };

  const updateField = (field: keyof TerceirizadaForm, value: string) => {
    if (field === 'cnpj') value = formatCNPJ(value);
    if (field === 'telefone') value = formatPhone(value);
    setForm(prev => ({ ...prev, [field]: value }));
  };

  const tipoLabel = (tipo: string) => tipo === 'cooperativa' ? 'Cooperativa' : 'Terceirizada';

  const filtered = terceirizadas.filter((p: any) =>
    ...
    <Layout>
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <h1 className="text-xl font-bold text-foreground">Terceirizadas / Cooperativas</h1>
          <Button onClick={() => setDialogOpen(true)} size="sm">
            <Plus className="h-4 w-4 mr-1" /> Novo Cadastro
          </Button>
        </div>

        <Input placeholder="Buscar por nome, CNPJ ou responsável..." value={search} onChange={(e) => setSearch(e.target.value)} className="max-w-sm" />
      ...
      <Dialog open={dialogOpen} onOpenChange={(open) => !open && closeDialog()}>
        <DialogContent className="max-w-lg">
          <DialogHeader><DialogTitle>{editId ? 'Editar Registro' : 'Novo Cadastro'}</DialogTitle></DialogHeader>
          <div className="grid gap-4 py-2">
            ...
            <div className="space-y-1.5">
              <Label>E-mail</Label>
              <Input type="email" placeholder="contato@empresa.com.br" value={form.email} onChange={(e) => updateField('email', e.target.value)} />
            </div>
          </div>
          <DialogFooter>
...
export default CadastroTerceirizadas;
